#!/usr/bin/env python3
"""Generate notebooks/08_novel_analysis.ipynb from nb07 patterns (novel only)."""
import json
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
NB07 = ROOT / "notebooks" / "07_overnight_analysis.ipynb"
OUT = ROOT / "notebooks" / "08_novel_analysis.ipynb"
...
md("""# toy_task — Novel-stimulus analysis (nb08)

Focused analysis of the overnight sweep **restricted to `stimulus_regime=novel`**
(Holton-style rule transfer on genuinely distinct stimulus sets). The shared-stimulus
(output-remap) condition is excluded entirely.

Data: `data/runs_overnight/` (same cache as nb07, filtered after load).
Figures: `figures/nb08/`.
""")
...
FIG_DIR = PROJECT_ROOT / "figures" / "nb08"
FIG_DIR.mkdir(parents=True, exist_ok=True)
CACHE = PROJECT_ROOT / "data" / "overnight_summary.pkl"
EPOCHS = 8000
REGIME = "novel"   # nb08: novel stimuli only
...
print("regime:", REGIME, "| cache:", OUT_ROOT.exists())
print(f"γ={GAMMAS} | H={HIDDEN_SIZES} | ref slice: H={REF_H}, scope={REF_SCOPE}")
""")

md("""## 2. Load runs → cached summary (novel only)

Reuses the nb07 summary cache, then filters to `stimulus_regime=novel`.
""")

loader_src = "".join(src["cells"][4]["source"])
loader_src = loader_src.replace(
    "stimulus_regime=regime).run_id()",
    "stimulus_regime=regime, angle_mode=ANGLE_RANDOM).run_id()",
)
loader_src = loader_src.replace(
    ...
    'run_ids = [os.path.basename(os.path.dirname(p))\n'
    '               for p in glob.glob(str(OUT_ROOT / "*" / "config.json"))\n'
    '               if "_angeven_" not in os.path.basename(os.path.dirname(p))]',
)
loader_src = loader_src.replace(
    "df = build_summary()",
    'df = build_summary()\ndf = df[df.regime == REGIME].copy()\nprint(f"filtered to legacy random / {REGIME}: {len(df)} runs")',
)
code(loader_src)
...
code("""def load_curve(arch, off, g, si, H=REF_H, scope=REF_SCOPE, seed=0):
    rid = RunConfig(arch=arch, hidden_size=H, gamma=g, similarity=SIMILARITY_GRID[si],
                    ...
                    similarity_index=si, init_scope=scope, stimulus_regime=REGIME, angle_mode=ANGLE_RANDOM).run_id()
...
def build_timecourse(H=REF_H, scope=REF_SCOPE, sim_idx=cond_idx, seeds=(0, 1, 2), cache=True):
    path = PROJECT_ROOT / "data" / f"overnight_tc_{REGIME}_H{H}_{scope}.pkl"
