"""
Temporal data processing for RNNs (optional multi-step input).
"""
import torch
import numpy as np
...
def flatten_list(l):
    """Flattens recursively a list of lists."""
...
def add_structured_noise(data, n_samples=5, noise_ratio=0.9):
    """
    Generate a noisy version of a batch of samples.
    """
...
def add_temporal_noise(
    ...
    """Create a noisy temporal input sequence."""
...
def temporal_data(data, nb_steps=2, flatten=True, noise_ratio=None, random_start=False):
    ...
    if flatten and not is_list:
        if data.dim() == 2:
            # 2D (n_items, input_dim): do not flatten with start_dim=2 (would be out of range)
            pass
        elif data.shape[1] == 1:
...
def process_data(data, data_config):
    """Process data for temporal model (nb_steps, noise_ratio, random_start from config)."""
    nb_steps = data_config["nb_steps"]
    noise_ratio = data_config["noise_ratio"]
    random_start = data_config["random_start"]
