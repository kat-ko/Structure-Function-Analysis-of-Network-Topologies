...
from a1b2.models.surrogate import super_spike


class Quantization:
    """
    Simple quantization class to replace quant.quantization.Quantization
    Quantizes input tensor values to the nearest values in quant_values.
    """
    def __init__(self, quant_values):
        """
        Args:
            quant_values: numpy array or list of quantization levels
        """
        self.quant_values = torch.tensor(quant_values, dtype=torch.float32)
    
    def __call__(self, x):
        """
        Quantize input tensor to nearest quant_values.
        
        Args:
            x: input tensor to quantize
            
        Returns:
            Quantized tensor with values from quant_values
        """
        if not isinstance(x, torch.Tensor):
            x = torch.tensor(x)
        
        # Move quant_values to same device as x
        quant_values = self.quant_values.to(x.device)
        
        # Flatten x for processing, then reshape back
        original_shape = x.shape
        x_flat = x.flatten()
        
        # Find nearest quantization value for each element
        # Expand dimensions for broadcasting: (n_quant, n_elements)
        distances = torch.abs(x_flat.unsqueeze(0) - quant_values.unsqueeze(1))
        indices = torch.argmin(distances, dim=0)
        
        # Get quantized values and reshape back
        quantized_flat = quant_values[indices]
        return quantized_flat.reshape(original_shape)

# Keys must match config cell_type (e.g. "RNN", "GRU"); str(nn.RNN) is "<class '...'>", not "RNN".
cell_types_dict = {"RNN": nn.RNN, "GRU": nn.GRU, "RNNCell": nn.RNNCell, "GRUCell": nn.GRUCell}
# #region agent log
def _dbg_log(msg, data, hypothesis_id):
    import json
    try:
        with open("/home/kathrin/workspace/Structure-Function-Analysis-of-Network-Topologies/.cursor/debug.log", "a") as f:
            f.write(json.dumps({"location": "community.py", "message": msg, "data": data, "hypothesisId": hypothesis_id}) + "\n")
    except Exception:
        pass
# #endregion
...
class Community(nn.Module):
    ...
    def __init__(
        ...
        # #region agent log
        _dbg_log("cell_types_dict keys and self.cell_type", {"cell_type": self.cell_type, "dict_keys": list(cell_types_dict.keys()), "cell_type_in_dict": self.cell_type in cell_types_dict, "runId": "post-fix"}, "H1")
        # #endregion
...
def init_model(config, device=torch.device("cpu")):
    """
    Build Community from config; optional optimizer and scheduler.
    For use with Experiment/retraining (dynspec-style). Does not set readout dimensions from task.
    """
    model = Community(config).to(device)
    optim_config = config.get("optim", {})
    gamma = optim_config.pop("gamma", None)
    optimizer = torch.optim.AdamW(model.parameters(), **optim_config) if optim_config else torch.optim.AdamW(model.parameters(), lr=1e-3)
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma) if gamma else None
