"""
Retraining and ablations metrics for Community models (dynspec-style).
"""
import torch
...
from a1b2.training.community import train_community, test_community, is_notebook, get_acc
from a1b2.models.community import Readout, init_model
...
def create_retraining_model(
    ...
    model, _, _ = init_model(config, device=device)
    model.load_state_dict(state_dict_copy)
    retraining_config = copy.deepcopy(config)
    ...
    out_masks = torch.stack(
        [
            torch.stack(
                [
                    readout_mask(model.n_modules, model.hidden_size, n_hid, ag_to_mask=i)
                    if n_hid is not None
                    else readout_mask(model.n_modules, model.hidden_size, n_classes, ag_to_mask=i)
    ...
    for n, p in model.named_parameters():
        if p.requires_grad and "readout" not in n:
...
def compute_retraining_metric(
    ...
    **kwargs,
    ...
    retraining_optimizer = torch.optim.AdamW(
        model.parameters(), **retraining_config.get("optim", {"lr": 1e-3})
    )
...
def compute_ablations_metric(
    ...
    ablations_config = copy.deepcopy(config)
