"""
Human participant analysis: transfer metrics, behavioral metrics.
"""
import pandas as pd
import numpy as np
...
def compute_transfer_humans(trial_df):
    ...
    transfer_df = pd.merge(final_a1, first_b, on=['participant', 'condition', 'study'])
    transfer_df['error_diff'] = transfer_df['accuracy_B'] - transfer_df['accuracy_A1']
    return transfer_df


def add_behav_metrics(group_df, trial_df):
    """Add behavioral metrics to group_df (lumper/splitter, summer accuracy, transfer, etc.)."""
    ...
    for p in group_df['participant'].unique():
        ...
        group_df.loc[group_df['participant'] == p, 'interference'] = 1 - group_df.loc[group_df['participant'] == p, 'A_weight_A2'].values[0].astype(np.float32)
