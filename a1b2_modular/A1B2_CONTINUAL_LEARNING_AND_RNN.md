...
**Simulation schema v2 (during-training core/comms):** Conditions with `log_during_training: true` (and/or `sim_schema_version: 2`) append a **`__s2`** segment to `run_id` so outputs stay in a **separate folder** from legacy runs. Saved `.npz` files gain optional **`during_core_per_module`**, **`during_comms_per_module`**, L2 summaries, optional **`during_comms_m0_over_m1_l2`**, aligned with trial order; see **[docs/RUN_NAMING.md](docs/RUN_NAMING.md)**. Optional **`during_log_post_step: false`** speeds logging by using one forward (pre-update activations). Example condition: `two_module_rnn_50_task_routed_trainlog_s2`. Hypothesis-oriented slices: **`a1b2.analysis.during_training`**.
