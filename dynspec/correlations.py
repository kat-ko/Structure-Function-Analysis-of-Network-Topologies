...
class CKA:
    ...
    Provides linear CKA similarity metric for comparing representations.
    """
    ...
    def linear_CKA(self, X, Y):
        ...
        Args:
            X (np.ndarray): First representation matrix (n_samples x n_features)
            Y (np.ndarray): Second representation matrix (n_samples x n_features)
        ...
        # Compute the numerator: ||Y^T X||_F^2 = trace((Y^T X)^T (Y^T X)) = trace(X^T Y Y^T X)
        # Using cyclic property: trace(X^T Y Y^T X) = trace(Y Y^T X X^T)
        numerator = np.trace(Y @ Y.T @ X @ X.T)
        
        # Compute the denominator: ||X^T X||_F * ||Y^T Y||_F
        # ||X^T X||_F^2 = trace((X^T X)^T (X^T X)) = trace(X^T X X^T X) = trace(X X^T X X^T)
        # ||X^T X||_F = sqrt(trace(X X^T X X^T))
        XX_norm_sq = np.trace(X @ X.T @ X @ X.T)
        YY_norm_sq = np.trace(Y @ Y.T @ Y @ Y.T)
        denominator = np.sqrt(XX_norm_sq * YY_norm_sq)
        
        # Handle edge case: if denominator is zero, return 0
        if denominator == 0:
            return 0.0
